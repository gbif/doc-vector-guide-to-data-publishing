...
There are four https://www.gbif.org/dataset-classes[dataset classes^] that are supported for publication in GBIF:
...
For more information, please check https://data-blog.gbif.org/post/choose-dataset-type[how to choose a dataset class on GBIF^].
...
Once a dataset class is selected, the data must be transformed to comply with a https://www.gbif.org/darwin-core[data standard^] that is accepted in GBIF, the most widely used standard in biodiversity is the https://www.tdwg.org/standards/dwc/[Darwin Core standard^], there are https://dwc.tdwg.org/terms/[more than 180 Darwin Core terms^] that are stable and provide vocabularies related to organisms and their related data. Most terms cover the wide range of variables present in vector collection, but some variables, such as environmental, genomic data, among others are best represented by https://rs.gbif.org/extensions.html[extensions^], which are sets of terms corresponding to a specific category of data.
...
[fig-03]
.Mapping species names. In this example, three different publishers refer to species names using different headers (lines 2). When mapping to DwC, species names become the term:dwc[dwc:scientificName] term.
...
* https://creativecommons.org/licenses/by-nc/4.0[CC BY-NC 4.0^]: data is available for any non-commercial use with appropriate attribution
...
Once the dataset is published, GBIF provides selected https://www.gbif.org/dataset/f0ccf436-e2e8-4f61-af19-753cdb73ca00/metrics[metrics^] about the datasets, https://www.gbif.org/dataset/f0ccf436-e2e8-4f61-af19-753cdb73ca00/activity[user download activity^] and https://www.gbif.org/resource/search?contentType=literature&gbifDatasetKey=f0ccf436-e2e8-4f61-af19-753cdb73ca00[cited reuses in published research and policy^].
...
Different metadata standards exist, and GBIF relies on the https://eml.ecoinformatics.org/[Ecological Metadata Language standard^] (EML) to record information about datasets using XML document types.

Entering the required information in the metadata section of the IPT metadata editor generates a metadata file that is included in the DwC-A file. This is an https://api.gbif.org/v1/dataset/83ab030b-d243-469c-8947-1ed487e06a80/document[example of a XML metadata file^].
...
[width="99%",cols="18%,19%,25%,30%,8%",options="header",]
|===
|*Section in the IPT* |*EML term* |*Definition* |*Example* |*Status*

|Basic Metadata |title |Title of the dataset |Mosquitoes (Diptera:
Culicidae) Distribution in Thailand |Required

| |metadataLanguage |- |English |

| |type |Please select dataset type from drop-down menu |Sampling Event
|

| |organizationName |Organization name responsible for the vector
collection. |Walailak University |Required

| | |The organization must be previously registered with GBIF. | |

| | |Corresponds to publishingOrganization in the IPT. | |

| |dataLanguage |- |English |

| |maintenanceUpdateFrequency |Choose from the menu or leave unknown.
|Unknown or Continually or Irregular etc. |

| | |Corresponds to updateFrequency in the IPT. | |

| |licensed[2] |Choose from three types, but recommended license is
CC-By 4.0 |Creative Commons Attribution (CC-BY 4.0) |Required

| | |Corresponds to dataLicense in the IPT. | |

| |abstract |A brief overview of the resource that is being documented.
|This dataset combines the occurrence records of mosquitoes from various
provinces across Thailand. |Required

| | |Corresponds to description in the IPT. | |

|Resource Contacts | |The list of contacts represents the people and
organizations that should be contacted to get more information about the
resource, that curate the resource or to whom putative problems with the
resource or its data should be addressed. |Last name Sukkanon |Required

| |contact | |Position: Assistant Professor |

| | |Corresponds to “"`resourceContact(s)`"” in the IPT. |Organization:
Walailak University |

| | |Required fields in the IPT: last name, position, organization | |

|Resource creators |creator |The list of creators represents the people
and organizations who created the resource, in priority order. The list
will be used to auto-generate the citation (if auto-generation is turned
on). |Last name Chareonviriyaphap |Required

| | | |Position: Professor |

| | |Corresponds to resourceCreator(s) in the IPT. |Organization:
Kasetsart University |

| | |Required fields in the IPT: Last name, Position, Organization,
E-mail | |

|Metadata providers |- |The list of metadata providers represents the
people and organizations responsible for producing the resource
metadata. |Last name Sukkanon |Recommended

| | | |Position: Assistant Professor |

| | |It is the metadataProvider(s) in the IPT. |Organization: Walailak
University |

| | |Required fields in the IPT: Last name, Position, Organization,
E-mail | |

|Geographic Coverage |coverage |A brief description of geographical
coverage. |Mosquitoes from Thailand. |Required

| | |Corresponds to geographicCoverage in the IPT. | |

|Keywords (3-5) |- |In the IPT: Keyword list[3]. |infectious diseases,
One Health, vectors, diseases names (e.g. West Nile Virus, dengue,
cutaneous leishmaniasis) |Recommended

| | |To improve discoverability we suggest making use of an extensive
list. | |

|Project Data |project |Metadata about the project that generated the
dataset. |Mosquitoes (Diptera: Culicidae) Distribution in Thailand
|Required

| | |Corresponds to projectData in the IPT. | |

| | |Please provide at least the title of the project. Additional fields
for identifier, description, funding, study area description or design
...
Corresponds to samplingMethods in the IPT. |…the collected mosquitoes were initially identified using well-established morphological keys.[4]
|Required
|===

[1] Example data were taken from the following published dataset: https://doi.org/10.15468/tbd7fz

[2] More information can be found here: https://www.gbif.org/terms

[3] Metadata descriptors for health: DeCS/MeSH, Global Index Medicus, Unified Medical Language System (UMLS)

[4] Extracts from the sampling method section from the dataset: https://doi.org/10.15468/tbd7fz 
